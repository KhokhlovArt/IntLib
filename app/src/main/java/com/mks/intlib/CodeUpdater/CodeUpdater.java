package com.mks.intlib.CodeUpdater;
...
import com.mks.intlib.CryptoProvider.CryptoProviderServicer;
import com.mks.intlib.FilesLoader.FilesLoader;
...
public class CodeUpdater {
    ...
    {
        INotificationViewer nv = new NotificationViewerGetter().getNotificationViewer(cnt);
        String current_version = nv.getVersion();
        String json_str = loadJSONFromAsset(cnt, C.ConfigFilePath(cnt));
        String res = null;
        JSONObject obj = null;
        try {
            ...
            JSONObject rootObj = getJsonObj(json_str);
            obj = rootObj.getJSONObject(C.JSON_KEY_VERSIONS);
                    ...
                    if (compareCodeVersion(current_version, key) > 0)
                    {
                        ...
                        if ( obj.get(key) instanceof JSONObject ) {
                            ...
                            if(isDeviceIDApprove && isApkPackageApprove && isVersion)
                            {
                                ...
                                SharedPreferencesServicer.setPreferences(cnt, C.SPF_SESSION_DEX_HASH, C.SPF_KEY_DEX_HASH, hash_code);
                                break;
                            }
                        }
                    }
...
//    }
    ...
    {
        return path; //RestServicer.getUrlToDownloadFile(cnt, path, comment, deviceID);
    }
